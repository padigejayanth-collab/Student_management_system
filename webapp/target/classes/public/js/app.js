...
function showAlert(message, type = 'success') {
    // Ensure a single alert container exists (fixed, top-right)
    let container = document.getElementById('app-alerts');
    if (!container) {
        container = document.createElement('div');
        container.id = 'app-alerts';
        // lightweight inline styling to avoid depending on CSS changes
    ...
    // Deduplicate identical alerts to avoid repeated popping
    const existing = Array.from(container.querySelectorAll('.alert')).find(a => {
        return a.dataset.msg === message && a.dataset.type === type;
    });
    if (existing) {
        // Optionally, we could refresh its timeout or briefly highlight it.
        return;
    }
    ...
    // Remove after timeout with a small fade (if CSS supports it); otherwise remove outright
    setTimeout(() => {
        try {
            alertDiv.remove();
        } catch (e) {
